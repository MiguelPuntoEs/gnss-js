...
import { keplerPosition, ecefToAzEl, computeDop } from '../src/orbit/index';
...
const HAS_CROSS_DATA =
  existsSync(CROSS_V2_FILE) &&
  existsSync(CROSS_V3_FILE) &&
  existsSync(CROSS_V4_FILE);
...
describe.skipIf(!HAS_V2_DATA)('RINEX 2 Navigation file parser', () => {
  ...
  it('parses first record correctly', () => {
    const g01 = result.ephemerides.find(
      (e) => e.prn === 'G01'
    ) as KeplerEphemeris;
...
describe('Navigation file parser', () => {
  ...
  it('parses GPS ephemerides', () => {
    ...
    expect(g14.af0).toBeCloseTo(2.86359339952e-4, 10);
    expect(g14.e).toBeCloseTo(3.8040264044e-3, 10);
    expect(g14.sqrtA).toBeCloseTo(5153.67734337, 3);
  });
...
describe('Orbit computation – nav file', () => {
  it('computes GPS satellite position from ephemeris', () => {
    const text = readFileSync(NAV_FILE, 'utf-8');
    const result = parseNavFile(text);
    const g14 = result.ephemerides.find(
      (e) => e.prn === 'G14'
    ) as KeplerEphemeris;
...
describe('Orbit computation', () => {
  it('computes azimuth and elevation', () => {
    ...
    const { az: _az, el } = ecefToAzEl(rxX, rxY, rxZ, rxX + 20e6, 0, 0);
...
describe.skipIf(!HAS_CROSS_DATA)(
  'Cross-version nav validation (2026/001)',
  () => {
    ...
    function buildEphMap(ephs: Ephemeris[]) {
      ...
      for (const e of ephs) {
        ...
        if (e.system === 'R' || e.system === 'S') {
          if (!glonass.has(k)) glonass.set(k, e);
        } else {
    ...
    beforeAll(() => {
      const m2 = buildEphMap(
        parseNavFile(readFileSync(CROSS_V2_FILE, 'utf-8')).ephemerides
      );
      const m3 = buildEphMap(
        parseNavFile(readFileSync(CROSS_V3_FILE, 'utf-8')).ephemerides
      );
      const m4 = buildEphMap(
        parseNavFile(readFileSync(CROSS_V4_FILE, 'utf-8')).ephemerides
      );
      v2k = m2.kepler;
      v3k = m3.kepler;
      v3g = m3.glonass;
      ...
      if (existsSync(CROSS_V2_GLO_FILE)) {
        v2g = buildEphMap(
          parseNavFile(readFileSync(CROSS_V2_GLO_FILE, 'utf-8')).ephemerides
        ).glonass;
      } else {
        v2g = new Map();
      }
    ...
    it('GPS satellite positions agree within 1 mm (V2 vs V3 vs V4)', () => {
      ...
      for (const key of commonKeys.slice(0, 5)) {
        ...
        const dist23 = Math.sqrt(
          (p2.x - p3.x) ** 2 + (p2.y - p3.y) ** 2 + (p2.z - p3.z) ** 2
        );
        const dist24 = Math.sqrt(
          (p2.x - p4.x) ** 2 + (p2.y - p4.y) ** 2 + (p2.z - p4.z) ** 2
        );
    ...
    it.each(['G', 'E', 'C', 'J'] as const)(
      '%s satellite positions agree within 1 mm (V3 vs V4)',
      (sys) => {
        const common: string[] = [];
        for (const [key, e] of v3k) {
        ...
        for (const key of common.slice(0, 5)) {
          ...
          const dist = Math.sqrt(
            (p3.x - p4.x) ** 2 + (p3.y - p4.y) ** 2 + (p3.z - p4.z) ** 2
          );
