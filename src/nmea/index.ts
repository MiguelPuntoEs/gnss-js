...
export function computeStats(fixes: NmeaFix[]): NmeaStats {
  ...
  const withTime = valid.filter((f) => f.time !== null) as (NmeaFix & {
    time: Date;
  })[];
  const withSat = valid
    .filter((f) => f.satellites !== null)
    .map((f) => f.satellites!);
  ...
  const speeds = valid
    ...
    .map((f) => f.speed! * 1.852); // knots → km/h
  ...
  return {
    ...
    avgSatellites:
      withSat.length > 0
        ? withSat.reduce((a, b) => a + b, 0) / withSat.length
        : null,
  };
}
...
export function parseNmeaFile(content: string): NmeaTrack {
  ...
  for (const raw of lines) {
    ...
    if (existing) {
      ...
      if (partial.alt !== undefined && partial.alt !== null)
        existing.alt = partial.alt;
      if (partial.satellites !== undefined && partial.satellites !== null)
        existing.satellites = partial.satellites;
      if (partial.fixQuality !== undefined)
        existing.fixQuality = partial.fixQuality;
      if (partial.speed !== undefined && partial.speed !== null)
        existing.speed = partial.speed;
      if (partial.course !== undefined && partial.course !== null)
