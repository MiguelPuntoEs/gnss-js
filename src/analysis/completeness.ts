...
import { BAND_LABELS, SYSTEM_NAMES } from '../constants/gnss';
...
export class CompletenessAccumulator {
  ...
  private data = new Map<
    string,
    Map<string, { seen: boolean; expected: number; present: number }>
  >();
  ...
  finalize(): CompletenessResult {
    ...
    const sigAgg = new Map<
      string,
      { expected: number; present: number; sats: Set<string> }
    >();
    ...
    const systems = [...systemSet].sort((a, b) => {
      return (
        (sysOrder.indexOf(a) === -1 ? 99 : sysOrder.indexOf(a)) -
