...
function gloDerivatives(
  ...
  const dvx =
    -mu_r3 * x +
    j2_term * x * ((5 * z * z) / r2 - 1) +
    OMEGA_E * OMEGA_E * x +
    2 * OMEGA_E * vy +
    ax;
  const dvy =
    -mu_r3 * y +
    j2_term * y * ((5 * z * z) / r2 - 1) +
    OMEGA_E * OMEGA_E * y -
    2 * OMEGA_E * vx +
    ay;
 ...
 */
export function glonassPosition(
...
function invert4x4(m: number[][]): number[][] | null {
  ...
  det = 1.0 / det;
  const result: number[][] = Array.from({ length: 4 }, () =>
    new Array(4).fill(0)
  );
  for (let i = 0; i < 4; i++) {
    for (let j = 0; j < 4; j++) {
 ...
 */
export function computeSatPosition(
  eph: Ephemeris,
  timeMs: number
): SatPosition {
...
export function ephInfoToEphemeris(info: EphemerisInfo): Ephemeris | null {
  ...
  if (sys === 'R') {
    ...
    if (info.x === undefined || info.y === undefined || info.z === undefined)
      return null;
  ...
  if (
    info.sqrtA === undefined ||
    info.eccentricity === undefined ||
    info.inclination === undefined ||
    info.omega0 === undefined ||
    info.argPerigee === undefined ||
    info.meanAnomaly === undefined ||
    info.toe === undefined ||
    info.week === undefined
  )
    return null;
...
export function computeLiveSkyPositions(
  ...
  const sinRxLat = Math.sin(rxLat),
    cosRxLat = Math.cos(rxLat);
  const sinRxLon = Math.sin(rxLon),
    cosRxLon = Math.cos(rxLon);
  ...
  for (const info of ephemerides.values()) {
    ...
    try {
      ...
      const north =
        -sinRxLat * cosRxLon * dx - sinRxLat * sinRxLon * dy + cosRxLat * dz;
      const up =
        cosRxLat * cosRxLon * dx + cosRxLat * sinRxLon * dy + sinRxLat * dz;
      const az = (Math.atan2(east, north) + TWO_PI) % TWO_PI;
      const el = Math.atan2(up, Math.sqrt(east * east + north * north));

      if (el > -0.05) {
        // slight margin below horizon
        result.push({
          prn: info.prn,
          az,
          el,
